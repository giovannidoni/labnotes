...
def filter_by_score_and_date(
    ...
    for item, date in zip(similar_items, dates):
        ...
        logger.debug(f"Item '{item.get('title', 'Unknown')[:50]}...': "
...
async def main_async():
    ...
    parser.add_argument("--prefer-score", action="store_true",
                       help="Prefer high score over recency (default: prefer recent)")
    parser.add_argument("--score-weight", type=float, default=0.3,
    ...
    try:
        ...
        logger.info(f"Deduplication complete: {len(deduplicated)} items retained, "

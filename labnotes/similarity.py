...
async def remove_similar_items(
    ...
    logger.info(f"Starting similarity-based deduplication on {len(items)} items "
    ...
    for i in range(len(valid_items)):
        for j in range(i + 1, len(valid_items)):
            ...
            if similarity >= similarity_threshold:
                ...
                logger.debug(f"Found similar items: '{valid_items[i].get('title', 'Unknown')[:50]}...' "
    ...
    for group in similar_groups:
        ...
        group_items.sort(
            key=lambda x: parse_date_for_comparison(x[1].get("date", "")),
            reverse=True
        )
        ...
        logger.debug(f"In similarity group, keeping most recent: "
    ...
    logger.info(f"Similarity-based deduplication complete: {len(final_items)} items retained, "
